...
import { FaUsers, FaEye, FaChartLine, FaMoneyBillWave, FaCheck, FaBackward, FaChevronLeft, FaChevronRight } from 'react-icons/fa';
...
function ChannelDetailPremiumPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [channel, setChannel] = useState<Channel | null>(null);
  const [selectedDate, setSelectedDate] = useState(22); // Выбранный день
  ...
  const fetchChannelDetails = () => {
    ...
    const mockChannel: Channel = {
      ...
      availableDates: [19, 20, 21, 22, 23, 24, 27, 28, 29, 30, 31]
    };
  ...
  return (
    <div className="channel-detail-screen premium">
      ...
      <div className="content">
        ...
        <div className="calendar-section">
          ...
          <h3 className="month-title">Май 2023</h3>
          ...
          <div className="calendar">
            ...
            <div className="days-grid">
              {Array.from({ length: 31 }, (_, i) => i + 1).map(day => {
                const isAvailable = channel.availableDates.includes(day);
      ...
      <div className="bottom-action-bar">
        <button className="action-button">
          Купить рекламное размещение на {selectedDate} мая
        </button>
      </div>
    </div>
  );
...
export default ChannelDetailPremiumPage; 